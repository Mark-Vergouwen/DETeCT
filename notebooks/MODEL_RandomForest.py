...
all_df = (all_df.pipe(lambda df: print(
...
### 2. Pipeline
pipeline, X_test_bal, y_test_bal, y_pred_bal, X_test_unbal, y_test_unbal, y_pred_unbal, feature_importances, top10_df, report_bal_df, report_unbal_df, (fig_imp, ax_imp, fig_cm, ax_cm, fig_cm_unbal, ax_cm_unbal) = src.classification_random_forest_CV.train_random_forest_classifier(all_df, random_state=42, real_world_weights = data_config["real_world_weights"], encode_categorical = True, param_grid = None, n_cv_folds = 5)

### 3. Saving
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S3_SMA{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S3_SMA{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S3_SMA{config["output_suffix"]}_pipeline.joblib')

#%% Implementation with SmartMeterAnalytics - YEARLY (with weather features) - INJECTION
AGGREGATION_LEVEL = "yearly"  
DATA_TYPE = "public" 
...
### 1. Loading data
all_df = src.data_loading_SMA.load_sma_features(weather_prefix=config["prefixes"]["sma_weather_inj"], feature_prefix=config["prefixes"]["sma_features_inj"], time_col = config.get("time_col"), path = data_config["paths"]["data_smafeatures"], DATA_TYPE = DATA_TYPE, label_map = data_config["label_map"])
...
all_df_renamed = all_df.rename(
    columns={c: f"{c}_inj" for c in all_df.columns if c != "ean_id" and c != 'label'}
)

### 2. Pipeline
pipeline, X_test_bal, y_test_bal, y_pred_bal, X_test_unbal, y_test_unbal, y_pred_unbal, feature_importances, top10_df, report_bal_df, report_unbal_df, (fig_imp, ax_imp, fig_cm, ax_cm, fig_cm_unbal, ax_cm_unbal) = src.classification_random_forest_CV.train_random_forest_classifier(all_df_renamed, random_state=42, real_world_weights = data_config["real_world_weights"], encode_categorical = True, param_grid = None, n_cv_folds = 5)

### 3. Saving
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S3_SMA_INJ{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S3_SMA_INJ{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S3_SMA_INJ{config["output_suffix"]}_pipeline.joblib')


#%% Implementation with SmartMeterAnalytics - YEARLY (with weather features) - OFFTAKE + INJECTION
AGGREGATION_LEVEL = "yearly"  
DATA_TYPE = "public" 
...
### 1b. Loading WITHDRAWALS data
all_df_w = src.data_loading_SMA.load_sma_features(weather_prefix=config["prefixes"]["sma_weather"], feature_prefix=config["prefixes"]["sma_features"], time_col = config.get("time_col"),  path = data_config["paths"]["data_smafeatures"], DATA_TYPE = DATA_TYPE, label_map = data_config["label_map"])

all_df_w = (all_df_w.pipe(lambda df: print(
        df.groupby("label")["ean_id"].nunique().loc[lambda s: s < 10]
        .rename("n_households")
        .reset_index()
        .to_string(index=False)
...
# 2. Merge
all_df_w_renamed = all_df_w.rename(
    columns={c: f"{c}_inj" for c in all_df_w.columns if c != "ean_id"}
)

merged_df = all_df.merge(all_df_w_renamed, on="ean_id", how="left")
merged_df = merged_df.drop(columns=["label_inj"])

# 3. Run pipeline
pipeline, X_test_bal, y_test_bal, y_pred_bal, X_test_unbal, y_test_unbal, y_pred_unbal, feature_importances, top10_df, report_bal_df, report_unbal_df, (fig_imp, ax_imp, fig_cm, ax_cm, fig_cm_unbal, ax_cm_unbal) = src.classification_random_forest_CV.train_random_forest_classifier(merged_df, random_state=42, real_world_weights = data_config["real_world_weights"],  encode_categorical = True, param_grid = None, n_cv_folds = 5)

# 4. Saving
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S3_SMA_BOTH{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S3_SMA_BOTH{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S3_SMA_BOTH{config["output_suffix"]}_pipeline.joblib')


#%% Implementation with SmartMeterAnalytics - MONTHLY (with weather features)
...
#%% Implementation with SmartMeterAnalytics - MONTHLY (with weather features) INJECTION
AGGREGATION_LEVEL = "monthly"
DATA_TYPE = "public"
...
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S3_SMA_INJ{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")  
report_bal_df.to_latex(f'F3_SMA_INJ{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S3_SMA_INJ{config["output_suffix"]}_pipeline.joblib')

#%% Implementation with SmartMeterAnalytics - WEEKLY (with weather features)
...
#%% Implementation with SmartMeterAnalytics - WEEKLY (with weather features) INJECTION
AGGREGATION_LEVEL = "weekly"
DATA_TYPE = "public"
...
# 3. Saving
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S1_tsf{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S1_tsf{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S1_tsf{config["output_suffix"]}_pipeline.joblib')


#%% Implementation with tsfeatures - MONTHLY
AGGREGATION_LEVEL = "monthly"  
DATA_TYPE = "private"
...
# ---  0. Load data with function ---
features_df = src.data_loading_tsfeatures.load_tsf_features(feature_prefix=config["prefixes"]["tsf"], time_col = config.get("time_col"), path = data_config["paths"]["data_tsfeatures"], DATA_TYPE = DATA_TYPE, label_map = data_config["label_map"])

features_df = (features_df.pipe(lambda df: print(
        df.groupby("label")["ean_id"].nunique().loc[lambda s: s < 10]
        .rename("n_households")
        .reset_index()
        .to_string(index=False)
...
pipeline, X_test_bal, y_test_bal, y_pred_bal, X_test_unbal, y_test_unbal, y_pred_unbal, feature_importances, top10_df, report_bal_df, report_unbal_df, (fig_imp, ax_imp, fig_cm, ax_cm, fig_cm_unbal, ax_cm_unbal) = src.classification_random_forest_CV.train_random_forest_classifier(features_df, random_state=42, real_world_weights = data_config["real_world_weights"],  encode_categorical = True, param_grid = None, n_cv_folds = 5)
...
#%% Implementation with tsfeatures - WEEKLY - does not exist
...
# ---  0. Loading data via function  ---
features_df = src.data_loading_IFEEL.load_ifeel_features(feature_prefix=config["prefixes"]["ifeel"], time_col = config.get("time_col"), path = data_config["paths"]["data_ifeelfeatures"], label_map = data_config["label_map"])
...
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S2_IFEEL{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S2_IFEEL{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S2_IFEEL{config["output_suffix"]}_pipeline.joblib')

#%% Implementation with IFEEL - MONTHLY
AGGREGATION_LEVEL = "monthly" 
DATA_TYPE = "private" 
...
#%% Implementation with IFEEL - WEEKLY
AGGREGATION_LEVEL = "weekly"  
DATA_TYPE = "public"
...
# 3. Saving
os.chdir(data_config["paths"]["output"])
fig_cm.savefig(f'S4_Fourier{config["output_suffix"]}_ConfusionMatrix.png', dpi=300, bbox_inches="tight")
report_bal_df.to_latex(f'S4_Fourier{config["output_suffix"]}_ClassificationReport.tex', float_format="%.3f")

os.chdir(data_config["paths"]["models"])
dump(pipeline, f'S4_Fourier{config["output_suffix"]}_pipeline.joblib')


#%% Implementation with Fourier - MONTHLY (with weather features)
AGGREGATION_LEVEL = "monthly"   
DATA_TYPE = "private"
...
#%% Implementation with Fourier - WEEKLY (with weather features)
AGGREGATION_LEVEL = "weekly"
DATA_TYPE = "private"
