...
print(f"Loaded DataFrame with shape {features_df.shape}")

# ---  2. Apply tSNE as exploratory data analysis ---
# Loops over possible perplexity values [5, 10, 20, 30, 40, 50]. Produces as output 2-dimensional tSNE plots, with silhouette scores
random_state = 100
perplexity_values = [5, 10, 20, 30, 40, 50]  # commonly meaningful values

for perplexity in perplexity_values:
    print(f"Running t-SNE with perplexity={perplexity}...")
    
    # Generate t-SNE visualization
    fig, ax, tsne_df = src.exploratory_tSNE_y.plot_tsne(
        features_df, random_state=random_state, perplexity=perplexity, standardise = False
    )
    
    # Define output filenames
    os.chdir(data_config["paths"]["output"])
    output_base = f"tSNE_tsfeatures_{random_state}_{perplexity}"
...
# ---  1. Fluvius data: load data --- 
features_df = src.data_loading_SMA.load_sma_features(weather_prefix=config["prefixes"]["sma_weather"], feature_prefix=config["prefixes"]["sma_features"], time_col = config.get("time_col"), path = data_config["paths"]["data_smafeatures"], DATA_TYPE = DATA_TYPE, label_map = data_config["label_map"])
...
for perplexity in perplexity_values:
    ...
    # Define output filenames
    os.chdir(data_config["paths"]["output"])
    output_base = f"tSNE_SMA_{random_state}_{perplexity}"
...
# ---  1. Fluvius data: load data --- 
features_df = src.data_loading_Fourier.load_Fourier_features(feature_prefix=config["prefixes"]["fourier"], time_col = config.get("time_col"), path = data_config["paths"]["data_fourierfeatures"], DATA_TYPE = DATA_TYPE, label_map = data_config["label_map"])
...
for perplexity in perplexity_values:
    ...
    # Define output filenames
    os.chdir(data_config["paths"]["output"])
    output_base = f"tSNE_Fourier_{random_state}_{perplexity}"
...
for perplexity in perplexity_values:
    ...
    # Define output filenames
    os.chdir(data_config["paths"]["output"])
    output_base = f"tSNE_IFEEL_{random_state}_{perplexity}"
...
#%% Implementation - updated with all features
AGGREGATION_LEVEL = "yearly"  # Options: "yearly", "monthly" or "weekly" 
DATA_TYPE = "public"  # Options: "private"  or "public"  or  "public2022"
...
print(f'- {"SMA":<12} : size {df_sma.shape}')
print(f'- {"IFEEL":<12} : size {df_ifeel.shape}')
print(f'- {"Fourier":<12} : size {df_fourier.shape}')
if AGGREGATION_LEVEL != "weekly":
    print(f'- {"TSF":<12} : size {df_tsf.shape}')
...
if AGGREGATION_LEVEL != "weekly":
    common_ean = set(df_sma["ean_id"]) & set(df_tsf["ean_id"]) & set(df_ifeel["ean_id"]) & set(df_fourier["ean_id"])
else:
    common_ean = set(df_sma["ean_id"]) & set(df_ifeel["ean_id"]) & set(df_fourier["ean_id"])

inter_df_sma = df_sma[df_sma["ean_id"].isin(common_ean)].reset_index(drop=True)
...
if AGGREGATION_LEVEL != "weekly":
    df_all = (inter_df_sma.merge(inter_df_tsf, on=merge_keys, how='inner').merge(inter_df_ifeel, on=merge_keys, how='inner').merge(inter_df_fourier, on=merge_keys, how='inner'))
else: 
    df_all = (inter_df_sma.merge(inter_df_ifeel, on=merge_keys, how='inner').merge(inter_df_fourier, on=merge_keys, how='inner'))
...
# ---  3. Apply tSNE as exploratory data analysis ---
# Loops over possible perplexity values [5, 10, 20, 30, 40, 50]. Produces as output 2-dimensional tSNE plots, with silhouette scores
...
for perplexity in perplexity_values:
    ...
    # Generate t-SNE visualization
    fig, ax, tsne_df = src.exploratory_tSNE_y.plot_tsne(
        df_all, random_state=random_state, perplexity=perplexity, standardise = True
    )
    
    # Define output filenames
    os.chdir(data_config["paths"]["output"])
    output_base = f"tSNE_comb_{random_state}_{perplexity}"
